...
Dump of assembler code for function main:
   ...
   0x08048885 <+12>:    mov    %eax,0x1c(%esp)            # saved argv (unused)
   0x08048889 <+16>:    mov    %gs:0x14,%eax
   0x0804888f <+22>:    mov    %eax,0x4c(%esp)            # stack canary
   0x08048893 <+26>:    xor    %eax,%eax
   0x08048895 <+28>:    push   %eax                       # junk
   0x08048896 <+29>:    xor    %eax,%eax
   0x08048898 <+31>:    je     0x804889d <main+36>        # always taken
   0x0804889a <+33>:    add    $0x4,%esp
   0x0804889d <+36>:    pop    %eax
                                                        # 0x2c(%esp)  char login[32]
                                                        # 0x28(%esp)  unsigned int serial
   0x0804889e <+37>:    movl   $0x8048ad4,(%esp)
   0x080488a5 <+44>:    call   0x8048590 <puts@plt>       # banner
   0x080488aa <+49>:    movl   $0x8048af8,(%esp)
   0x080488b1 <+56>:    call   0x8048590 <puts@plt>       # "*    level06    *"
   0x080488b6 <+61>:    movl   $0x8048ad4,(%esp)
   0x080488bd <+68>:    call   0x8048590 <puts@plt>       # banner
   0x080488c2 <+73>:    mov    $0x8048b08,%eax
   0x080488c7 <+78>:    mov    %eax,(%esp)
   0x080488ca <+81>:    call   0x8048510 <printf@plt>     # "-> Enter Login: "
   ...
   0x080488e7 <+110>:   call   0x8048550 <fgets@plt>      # fgets(login, 32, stdin)
   ...
   0x08048918 <+159>:   call   0x8048510 <printf@plt>     # "-> Enter Serial: "
   ...
   0x08048948 <+207>:   jne    0x8048969 <main+240>       # fail → return 1
...
Dump of assembler code for function auth:
   ...
   0x08048764 <+28>:    movb   $0x0,(%eax)               # login[idx] = 0
   ...
   0x0804878a <+66>:    jg     0x8048796 <auth+78>       # if (len <= 5) return 1
   0x0804878c <+68>:    mov    $0x1,%eax
   0x08048791 <+73>:    jmp    0x8048877 <auth+303>
   0x08048796 <+78>:    movl   $0x0,0xc(%esp)            # data
   0x0804879e <+86>:    movl   $0x1,0x8(%esp)            # addr = 1
   0x080487a6 <+94>:    movl   $0x0,0x4(%esp)            # pid
   0x080487ae <+102>:   movl   $0x0,(%esp)               # PTRACE_TRACEME
   0x080487b5 <+109>:   call   0x80485f0 <ptrace@plt>    # ptrace(0, 0, 1, 0)
   0x080487ba <+114>:   cmp    $0xffffffff,%eax
   0x080487bd <+117>:   jne    0x80487ed <auth+165>      # already traced → fail
   0x080487bf <+119>:   movl   $0x8048a68,(%esp)
   0x080487c6 <+126>:   call   0x8048590 <puts@plt>
   0x080487cb <+131>:   movl   $0x8048a8c,(%esp)
   0x080487d2 <+138>:   call   0x8048590 <puts@plt>       # "TAMPERING DETECTED"
   ...
   0x080487f6 <+174>:   movsbl %al,%eax                   # (signed char)login[3]
   0x080487f9 <+177>:   xor    $0x1337,%eax               # ^ 4919
   0x080487fe <+182>:   add    $0x5eeded,%eax             # + 6221293
   0x08048803 <+187>:   mov    %eax,-0x10(%ebp)           # hash
   ...
   0x0804881a <+210>:   jg     0x8048823 <auth+219>       # if (login[i] <= 31) return 1
   ...
   0x0804882c <+228>:   movsbl %al,%eax                   # (signed char)login[i]
   ...
   0x08048861 <+281>:   jl     0x804880f <auth+199>       # while (i < len)
   0x08048863 <+283>:   mov    0xc(%ebp),%eax             # serial
   0x08048866 <+286>:   cmp    -0x10(%ebp),%eax           # serial == hash ?
   0x08048869 <+289>:   je     0x8048872 <auth+298>
   0x0804886b <+291>:   mov    $0x1,%eax                  # fail
...
the programs takes as input a login and a serial,
then it verify if the serial is equal to the hashed login, if so you are authenticated

we just need to emulate the hash function in order to get the correct serial
